...
chrome.runtime.sendMessage({ r: 'getSettings' }, function (s: Settings) {
    ...
    if (!settings.isExcluded && !settings.isExcludedForTab && !settings.isPaused && !settings.isPausedForTab) {
        ...
        let observer = new WebKitMutationObserver(function (mutations: any) {
...
function DoWin(win: Window, winContentLoaded: boolean) {
    ...
    function DoIframe(iframe: HTMLIFrameElement) {
        ...
        if (!_win) return; //with iFrames it happens
    ...
    function CheckBgImg(this: HTMLImageElement) {
        if (this.height <= _settings.maxSafe || this.width <= _settings.maxSafe) ShowEl.call(this.owner);
        (<any>this).onload = undefined;
    };
...
function RemoveClass(el: HTMLElement, n: string) { //these assume long unique class names, so no need to check for word boundaries
    let oldClass = el.className, newClass = el.className.replace(new RegExp('\\b' + n + '\\b'), '');
    if (oldClass != newClass) {
        el.className = newClass;
    }
}
function AddClass(el: HTMLElement, c: string) {
    el.className += ' ' + c;
}
